...
ESCUELA_API = "https://api.escuelajs.co/api/v1/products"
DEFAULT_LIMIT = 220
MAX_LIMIT = 5000
DEFAULT_EN_BRANDS = {"Apple", "Nike", "Adidas", "Sony", "Samsung", "H&M", "ZARA"}
...
def http_get_json(url: str, timeout: int = 15) -> dict | list:
    trust_env = os.getenv("CRAWLER_TRUST_ENV", "false").strip().lower() in ("1", "true", "yes", "on")
...
def load_local_json(path: str) -> list[dict]:
    ...
    if isinstance(payload, dict):
        for key in ("products", "items", "data", "rows"):
            ...
            if isinstance(value, list):
                return value
    raise ValueError("JSON format unsupported: expected list or object with products/items/data/rows")
...
def parse_en_brand_whitelist() -> set[str]:
    configured = os.getenv("CRAWLER_EN_BRANDS", "")
    if not configured.strip():
        return set(DEFAULT_EN_BRANDS)
...
def localize_category_to_zh(category: str) -> str:
    clean = normalize_text(category).lower()
    if contains_chinese(clean):
        return normalize_text(category)
    for key, zh in CATEGORY_MAP.items():
        if key in clean:
            return zh
    return "精选商品"


def detect_product_type_zh(*texts: str) -> str:
    merged = " ".join(normalize_text(text).lower() for text in texts if text)
    for key, zh in PRODUCT_KEYWORD_MAP.items():
        if key in merged:
            return zh
    return "商品"


def detect_whitelisted_brand(title: str, whitelist: set[str]) -> str:
...
def localize_product_to_zh(record: dict, en_brand_whitelist: set[str]) -> dict:
    category_zh = localize_category_to_zh(record.get("category") or "")
    product_type_zh = detect_product_type_zh(
        record.get("name") or "",
        record.get("description") or "",
        record.get("selling_points") or "",
        category_zh,
    )
    brand = detect_whitelisted_brand(record.get("name") or "", en_brand_whitelist)

    if not contains_chinese(record.get("name") or ""):
        if brand:
            record["name"] = f"{brand} {product_type_zh}"
        else:
            record["name"] = f"{category_zh}{product_type_zh}"

    if not contains_chinese(record.get("description") or ""):
        record["description"] = f"{record['name']}，主打高性价比与日常实用，支持快速发货与售后保障。"

    if not contains_chinese(record.get("selling_points") or ""):
        record["selling_points"] = f"高性价比；{category_zh}优选；适合日常使用"

    if not contains_chinese(record.get("specs") or ""):
        record["specs"] = f"规格：{category_zh}标准款"

    if not contains_chinese(record.get("policy") or ""):
        record["policy"] = build_policy()

    record["category"] = category_zh

    raw_tags = [normalize_text(tag) for tag in (record.get("tags") or "").split(",") if normalize_text(tag)]
    raw_tags = [tag for tag in raw_tags if contains_chinese(tag) or tag in en_brand_whitelist or tag.lower() in {"crawler", "escuelajs", "jsonfile"}]
    if category_zh not in raw_tags:
        raw_tags.append(category_zh)
    if brand and brand not in raw_tags:
        raw_tags.append(brand)
    if "crawler" not in raw_tags:
        raw_tags.append("crawler")
    if "escuelajs" in (record.get("tags") or "") and "escuelajs" not in raw_tags:
        raw_tags.append("escuelajs")
    if "jsonfile" in (record.get("tags") or "") and "jsonfile" not in raw_tags:
        raw_tags.append("jsonfile")
    record["tags"] = ",".join(dict.fromkeys(raw_tags))
    return record


def normalize_category(value) -> str:
    if isinstance(value, dict):
        return normalize_text(value.get("name") or value.get("slug") or "")
    return normalize_text(value)


def normalize_image(item: dict) -> str:
    direct = normalize_text(item.get("image") or item.get("image_url") or item.get("thumbnail") or item.get("商品图片") or "")
    if direct:
        return direct
...
def normalize_product_record(source: str, item: dict, now: datetime, data_source: str = "CRAWLER") -> dict:
    ...
    if not source_product_id:
        fingerprint = "|".join(
            [
                title,
                category,
                normalize_text(str(item.get("price") or item.get("商品价格") or "")),
                normalize_text(str(item.get("店铺名称") or "")),
                normalize_text(str(item.get("商品图片") or "")),
    ...
    price = to_decimal(item.get("price") or item.get("current_price") or item.get("sale_price") or item.get("商品价格"))
    image_url = normalize_image(item)
    source_hint = normalize_text(item.get("source") or "")
    updated_at = parse_update_time(item.get("update_time"), now)

    paid_count = to_int(item.get("已付款人数") or item.get("paid_count"))
    ...
    if not category:
        category = infer_category(title, raw_desc, specs, selling_points, normalize_text(item.get("_sheet_name") or ""))
    ...
    tags = ",".join(filter(None, [tags_value, category, source_hint, "crawler", source]))
...
def build_vector_row(product: dict) -> tuple[str, str, int, str, str, str, datetime]:
    ...
    chunk_text = "\n".join([
        ...
        f"source={normalize_text(product.get('data_source') or 'CRAWLER')}",
    ])
    metadata = {
        "product_id": product_id,
        "update_time": updated_at.isoformat() if updated_at else "",
        "category": normalize_text(product.get("category")),
        "source": normalize_text(product.get("data_source") or "CRAWLER"),
    ...
    return (
        ...
        normalize_text(product.get("data_source") or "CRAWLER"),
        updated_at,
    )


def crawl_products(limit: int, input_file: str | None = None) -> list[dict]:
    now = datetime.now(timezone.utc)
    en_brand_whitelist = parse_en_brand_whitelist()

    if input_file:
        local_items = load_local_data(input_file)
        source_key = Path(input_file).suffix.lower().replace(".", "") or "local"
        data_source = "XLSX" if source_key == "xlsx" else "CRAWLER"
        products = [
            localize_product_to_zh(normalize_product_record(source_key, item, now, data_source=data_source), en_brand_whitelist)
    ...
    try:
        payload = http_get_json(f"{ESCUELA_API}?limit={min(limit, 200)}", timeout=20)
        if isinstance(payload, list):
            products.extend(
                localize_product_to_zh(normalize_product_record("escuelajs", item, now), en_brand_whitelist)
                for item in payload
                if isinstance(item, dict) and item.get("id") is not None
            )
    except RequestException as exc:
        print(f"[WARN] escuelajs fetch failed: {exc}")

    if not products:
        raise RuntimeError(
            "No products crawled from escuelajs. If your environment uses a problematic proxy/TLS path, "
            "set CRAWLER_TRUST_ENV=true or use --input-json <your-file>."
        )
...
def clear_crawler_data(conn):
    with conn.cursor() as cur:
        cur.execute("DELETE FROM product_vectors WHERE source = 'CRAWLER' OR product_id IN (SELECT id FROM products WHERE COALESCE(data_source, 'MANUAL') = 'CRAWLER')")
        cur.execute("DELETE FROM cart_items WHERE product_id IN (SELECT id FROM products WHERE COALESCE(data_source, 'MANUAL') = 'CRAWLER')")
        cur.execute("DELETE FROM product_views WHERE product_id IN (SELECT id FROM products WHERE COALESCE(data_source, 'MANUAL') = 'CRAWLER')")
        cur.execute("DELETE FROM products WHERE COALESCE(data_source, 'MANUAL') = 'CRAWLER'")
    conn.commit()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Lightweight compliant ecommerce crawler + vector-ready normalization")
    parser.add_argument("--limit", type=int, default=DEFAULT_LIMIT, help="Total products to ingest, recommended 200~300")
    parser.add_argument("--replace", action="store_true", help="Clear existing crawler data before ingest")
    parser.add_argument("--dry-run", action="store_true", help="Only crawl and print summary, do not write DB")
    parser.add_argument("--input-json", type=str, default="", help="Import products from local JSON/CSV file instead of network crawling")
    parser.add_argument("--input-xlsx", type=str, default="", help="Import products from local XLSX file")
    parser.add_argument("--replace-source", type=str, default="", help="Clear existing data by data_source before ingest, e.g. XLSX")
...
def main():
    ...
    elif args.input_json and args.input_json.strip():
        input_file = args.input_json.strip()

    products = crawl_products(limit, input_file=input_file or None)
    ...
    try:
        ...
        if args.replace_source and args.replace_source.strip():
            ...
            conn.commit()
        if args.replace:
            clear_crawler_data(conn)
        upsert_products(conn, products)
        upsert_product_vectors(conn, products)
        current_source = normalize_text(products[0].get("data_source") if products else "CRAWLER") or "CRAWLER"
